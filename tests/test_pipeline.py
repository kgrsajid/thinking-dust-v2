...
from td.perception.hdc import build_default_vocabulary, generate_hypervector
...
class TestPipeline:
    ...
    def test_decide_unknown_escalates(self, pipeline):
        decision = pipeline.decide("Plan a birthday party for my cat")
        assert isinstance(decision, TDDecision)
        # Novel input should have low confidence
        assert decision.confidence.decision in ["confirm", "escalate"]

    def test_learn_and_retrieve(self, pipeline):
        ...
        # Verify MHN has patterns now
        assert len(pipeline.mhn) > 0

    def test_learn_correction(self, pipeline):
    ...
    def test_latency_under_50ms(self, pipeline):
        ...
        # Generous threshold for test environments
        assert elapsed_ms < 500  # Should be <5ms on real hardware
