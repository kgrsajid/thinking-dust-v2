...
### The 10K Synthetic Corpus

BEAGLE word vectors are trained on `data/synthetic_corpus_10k.txt` — a 10,000-sentence synthetic corpus covering programming, science, geography, technology, and general knowledge.

**How it was generated:**
- **Model:** Kimi K2.6 (Moonshot AI)
- **Prompt:** "Generate 10,000 diverse English sentences covering: programming (Python, JavaScript, Go, Ruby, Swift, C++, Rust), science (chemistry, biology, physics, astronomy), geography (countries, capitals, rivers, continents), technology (networks, databases, DevOps, cloud, APIs), general knowledge (history, music, sports, literature). Each sentence should be a simple declarative statement or question. No duplicates. One sentence per line."
- **Post-processing:** None. Raw output committed directly.
- **Date:** 2026-07-01

**To reproduce:**
```bash
# Using any LLM with the above prompt, save output to:
data/synthetic_corpus_10k.txt
# Then train BEAGLE:
python3 -c "
...
**Corpus stats:**
- 10,093 sentences (original 10K + 93 domain-expansion sentences added 2026-07-08)
- 2,258 unique words
- Training time: ~1.4 seconds on CPU
- Output: `data/word_vectors_10k.pkl` (~19MB)

**Domain coverage (verified):**
- Programming: Python, JavaScript, Go, Ruby, Swift, C++, Rust, databases, APIs
- Science: chemistry, biology, physics, astronomy
- Geography: countries, capitals, rivers, continents
- Technology: networks, DevOps, cloud, mobile, wireless
- Biology: cell, organelle, membrane, nucleus, mitosis, DNA
- Prison: prisoner, cell, jail, guard, inmate, locked
- Phone: cell phone, mobile, wireless, cellular, tower
- Finance: bank, loan, mortgage, interest, teller
- Food: apple, fruit, pie, orchard, juice

**Key property:** Words that appear in similar contexts get similar vectors. "capital" ~ "city" (0.3+), "sort" ~ "arrange" (0.6+), "debug" ~ "fix" (0.65+), "prison" ~ "prisoner" (0.36), "phone" ~ "wireless" (0.52). Unrelated words have near-zero similarity.

**Known limitation:** BEAGLE is a STATIC word vector model — one vector per word regardless of context. This is insufficient for WSD (see ARCHITECTURE.md Section 6 Limitations). Modern WSD requires contextualized embeddings (BERT).
...
| Signal | When Used | Example | Research Backing |
|--------|-----------|---------|-----------------|
| **`is_a` object comparison** | Primary — for type declarations | "cell is_a room" vs "cell is_a organelle" → different senses | Logical principle: different type = different sense |
| **LOTG subsumption** | Compatible types | "city" ⊑ "place" → same sense | OWL 2 rdfs:subClassOf (W3C, 2009) |
| **Morphological prefix** | Related terms | "organelle" vs "organism" → share "organ" → same sense | String overlap heuristic |
| **BEAGLE context routing** | Non-`is_a` facts | "cell part_of prison" → routes to prison sense | Jones & Mewhort (2007) — context vectors |

**Why `is_a` is the primary signal (not BEAGLE context):**

BEAGLE (Jones & Mewhort, 2007) is a **static** word vector model — one vector per word regardless of context. For WSD, we need **contextualized** representations (different vectors for the same word in different contexts).

Measured on the 10K corpus:
- SAME sense teach sentences: cosine similarity ~0.0 ± 0.03
- DIFFERENT sense teach sentences: cosine similarity ~0.0 ± 0.03
- **Separation: ~0.0** (no discriminative power)

Modern WSD research confirms this limitation:
- **Sumanathilaka et al. (2026):** Uses sentence-transformers (BERT), not static vectors
- **Navigli (AAAI 2026):** "Contextualized embeddings needed; static vectors insufficient"
- **Mosolova et al. (ACL 2025):** "In the LLM era, WSI remains unsolved"

**What would fix this:** SpaCy dependency-based context extraction (use syntactic neighbors, not all words) or BERT contextualized embeddings (requires 110M+ params, violates TD v2 constraints).
