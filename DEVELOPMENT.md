...
**How it was generated:** The corpus was generated by an LLM (Kimi K2.6) as diverse question-answer pairs and declarative sentences. It covers:
- Programming concepts (Python, JavaScript, Go, Ruby, Swift, C++)
- Science facts (chemistry, biology, physics, astronomy)
- Geography (countries, capitals, rivers, continents)
- Technology (networks, databases, DevOps, cloud)
- General knowledge (history, music, sports, literature)
...
**Why synthetic?** BEAGLE needs diverse co-occurrence statistics to develop meaningful word vectors. A synthetic corpus ensures coverage of all topic areas without copyright concerns. The diversity is more important than the quality of individual sentences.

**Key property:** Words that appear in similar contexts get similar vectors. "capital" is similar to "city" (0.3+), "sort" is similar to "arrange" (0.6+), "debug" is similar to "fix" (0.65+). Unrelated words have near-zero similarity.

**Scaling plan:** The corpus should be expanded to 100K-1M sentences for better coverage. Current limitations:
- Some domain-specific words have low vector quality
- Polysemous words (cell, bank, python) need more context diversity
- See Roadmap section for expansion plan
